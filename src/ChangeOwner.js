...
export default class ChangeOwner extends React.Component {
    ...
    onFormSubmit(e) {
        ...
        axios.put('http://'+  process.env.API_HOST  +':'+ process.env.API_PORT+'/cars', {
            key: this.state.key,
    ...
    componentDidMount() {
        this.props.setLoading(true);
        axios.get('http://'+  process.env.API_HOST  +':'+ process.env.API_PORT+'/cars/' + this.props.match.params.key).then(res => {
            this.props.setLoading(false);
            if (res.data.status) {
